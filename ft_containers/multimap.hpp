...
class multimap {
 ...
 public:
  ...
  multimap(InputIter first, InputIter last) : t() { t.insert_range_equal(first, last); }
  ...
  iterator
  insert(iterator position, const value_type &x) { return t.insert_equal(position, x); }

  template<typename InputIterator>
  void
  insert(InputIterator first, InputIterator last) { t.insert_range_equal(first, last); }
  ...
  value_compare
  value_com() const { return value_compare(t.key_comp()); }
...
inline bool
operator==(const multimap<Key, Value, Compare> &x,
		   const multimap<Key, Value, Compare> &y) { return x.t == y.t; }
...
inline bool
operator!=(const multimap<Key, Value, Compare> &x,
		   const multimap<Key, Value, Compare> &y) { return !(x == y); }

template<typename Key, typename Value, typename Compare>
inline bool
operator>(const multimap<Key, Value, Compare> &x,
		  const multimap<Key, Value, Compare> &y) { return y < x; }
...
template<typename Key, typename Value, typename Compare>
inline void
swap(multimap<Key, Value, Compare> &x, multimap<Key, Value, Compare> &y) { x.swap(y); }
}
