...
class map {
 ...
 public:
  ...
  map(InputIter first, InputIter last) : t() { t.insert_range_unique(first, last); }
  ...
  insert(const value_type &x) { return t.insert_unique(x); }

  iterator
  insert(iterator position, const value_type &x) { return t.insert_unique(position, x); }

  template<typename InputIterator>
  void
  insert(InputIterator first, InputIterator last) { t.insert_range_unique(first, last); }
...
template<typename Key, typename Value, typename Compare>
inline bool
operator!=(const map<Key, Value, Compare> &x,
		   const map<Key, Value, Compare> &y) { return !(x == y); }
...
inline bool
operator<=(const map<Key, Value, Compare> &x,
		   const map<Key, Value, Compare> &y) { return !(y < x); }
...
template<typename Key, typename Value, typename Compare>
inline void
swap(map<Key, Value, Compare> &x, map<Key, Value, Compare> &y) { x.swap(y); }
}
